...
Phases 0–18 substantially complete. The workspace has 13 crates (`fixed_math`, `sim`, `render`, `input_touch`, `input_desktop`, `net`, `replay`, `replay_sync`, `sync_test`, `replay_viewer`, `ice_vendor`, `tape_drop`, `app`) with 602 passing tests, clippy-clean, and cross-platform determinism matrix green. Full gameplay is implemented: movement, dash + i-frames, boomerang throw/ricochet/recall/catch, hits/death/respawn, round flow (first-to-5), replay codec + viewer with scrub, touch input, **desktop keyboard input (dev/testing only)**, **live Matchbox WebRTC netplay (P2P session swap, desync detection, forfeit — loopback-verified)**, sprite animation + particles + 16-color palette, the **art 2.0 overhaul** (3-direction sprite atlas: side/back/front), **seven arenas (Anchor/Crossing/Reliquary/Pit/Vigil/Gallery/Forest)** with spreading fire, burnable groves, per-arena floor compositions/cover skins/ambient motes, **pickups + perfect catch (all six modifier behaviors)**, the **game-feel layer** (screen shake, kill flash, kill-cam, synthesized audio, Android haptics, deterministic rematch), the **depth-duel camera** (WORLD_TILT_Y=0.75 tabletop perspective, Y-axis spawns, per-client PerspectiveFlip for online), a Title/lobby screen plus the **arena roster** (a full select screen — seven tables with their real floors and rule tags; the pick persists and, online, rides the room name so both peers structurally agree), persisted JSON settings, the **social layer** (reliable side-channel: install-id + dialed CURSTAG names, per-opponent grudge ledger, consent-gated RUN IT BACK rematch, clean leave, away-grace with honest forfeit blame), **TURN relay config** for carrier-NAT traversal, **dual-signed match results** (ed25519 keys minted beside the install-id, canonical `MatchStatement` swapped over the side channel on score-decided matches, `.attest.json` beside the tape, verified offline by `replay_sync --attest`), the **share loop** (tape_drop relay + SHARE QR on summary/theater + the **web theater**: the full app on wasm32/WebGL2, GitHub Pages deploy, `#watch=<id>` playback verified headless, wasm checksum lane as the 5th determinism lane), the **on-device replay theater** (REPLAYS screen: list, play, scrub, speed through the live presentation), the **rivalry home** (RIVALS screen off the Title: ranked rivalry rows with standing/streak/last-met, per-rival detail with sealed-result count and the rivalry's own tape ring playable in place, milestone meetings flaring the dark beyond on GO, and the cosmetic stream pair-reseeded online so each rivalry's table carries its own marks), the **shade** (SPAR THEIR SHADE on a rival's detail at three ring tapes: input-stream habits fitted onto the bot's own knobs, the ghost wearing the rival's full mint and honest SHADE naming, moving neither the rivalry nor the gauntlet tier), the **gauntlet** (persistent practice-bot ladder), the pip-slam score beat + victory pose, a **southpaw** touch layout, and the **sit-down ritual** (the private dial shows its join-link QR; the other phone's system camera opens the join page, whose OPEN IN 2-TOP button deep-links `twotop://join/<CODE>-<arena>` through the manifest's intent filter and a JNI launch-URI read into the dial and the arena pick — zero typing to land both phones at one table). `sim::SIM_VERSION` is `15`. The product is a **phone-vs-phone online fighter** — each player runs the APK on their Android device, taps Play, and the signaling server pairs them for a P2P match. The desktop build (`cargo run -p app`) is a dev/testing tool (local couch-versus with split keyboard). See `PLAYBOOK.md` for the end-to-end phone setup. The active workstream is the **NORTH program** — the ultimate-form design in `docs/NORTH.md` (six pillars, all sim-neutral: rivalry home, signed results, web theater, minted demons, shades, sit-down ritual), executed per `docs/plans/NORTH_PLAN.md`.
...
1. `SyncTestSession` (`crates/sync_test`) — every CI job, `check_distance: 16` (the live prediction window), catches single-machine non-determinism.
