...
Phases 0–17 substantially complete. The workspace has 11 crates (`fixed_math`, `sim`, `render`, `input_touch`, `input_desktop`, `net`, `replay`, `replay_sync`, `sync_test`, `replay_viewer`, `app`) with 332 passing tests, clippy-clean, and cross-platform determinism matrix green. Full gameplay is implemented: movement, dash + i-frames, boomerang throw/ricochet/recall/catch, hits/death/respawn, round flow (first-to-5), replay codec + viewer with scrub, touch input, **desktop keyboard input (PC local couch versus)**, net crate with lobby FSM, sprite animation + particles + 16-color palette, the **art 2.0 overhaul**, **three arenas (Phase 16: Anchor/Crossing/Reliquary)**, and **pickups + perfect catch (Phase 17: all six modifier behaviors — Fire/Heavy/Bouncy/Curve/Multishot/Phantom)**. Remaining work is tracked in `docs/plans/COMPLETION_PLAN.md`: pickup presentation art (Task 3.4), the PC + Web platform track (MP), real netplay wiring (Phase 12 completion), polish (Phase 18), and release tag.
